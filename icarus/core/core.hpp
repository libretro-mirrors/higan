struct Icarus {
  ...
  virtual auto read(const string& pathname) -> vector<uint8_t> {
    return file::read(pathname);
  }

  virtual auto readable(const string& pathname) -> bool {
  ...
  virtual auto directory_exists(const string& filename) -> bool {
